...
class DriverProfileProvider with ChangeNotifier {
  ...
  DriverProfileProvider({required this.apiConsumer}) {
    getProifleData();
  }

  getProifleData() async {
    final data = await apiConsumer.get(
      '${EndPoints.getDriverProfile}${await locator<CacheHelper>().getData(key: "id")}',
    );
  ...
  updateProfile({required BuildContext context}) async {
    ...
    try {
      final data = await apiConsumer.put(
          '${EndPoints.getDriverProfile}${locator<CacheHelper>().getData(key: "id")}',
          data: {
